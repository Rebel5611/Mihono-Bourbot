...
class Context:
    ...
    def notify_hints(self, team: int, hints: typing.List[Hint], only_new: bool = False,
        ...
        for spectator in self.spectators:
            if spectator.no_text:
                continue

            # initialize tracking
            seen = self.spectator_hints.setdefault(spectator, set())

            # filter only unseen hints
            new_hints = []
            for hint, msg in spectator_concerns:
                if hint not in seen:
                    seen.add(hint)
                    new_hints.append(msg)

            if new_hints:
                async_start(self.send_msgs(spectator, new_hints))
